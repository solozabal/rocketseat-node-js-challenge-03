// api/src/modules/orgs/routes.ts
...
export async function orgRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/orgs', async (request, reply) => {
    ...
    try {
      ...
      // create token using helper (expires in 24h)
  ...
  // GET /orgs/me - protected route using authenticate preHandler
  fastify.get('/orgs/me', { preHandler: fastify.authenticate }, async (request: any, reply) => {
    // At this point, request.org should be set by authenticate
    if (!request.org) {
